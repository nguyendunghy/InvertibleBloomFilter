    ...
    public void performIncrementalSync() {
        ...
        if (result.type == SchemaChangeType.UNKNOWN_CHANGE) {
            ...
            if (result.type != SchemaChangeType.NO_CHANGE) {
                LOG.info(String.format("Schema change (%s) detected for table %s", result.type.getDescription(), tableInfo.getTableRef()));
    ...
    private void updateTable() {
        ...
        try {
            ...
            if (!result.getSucceeded()) {
                ...
                resyncTable(String.format("Failed to decode IBF result for table [%s] so we need to resync it", tableInfo.getTableRef()));
            } else {
                LOG.info("Processing changes for table " + tableInfo.getTableRef());
                //Process Result after calling diff
            ...
            tableState.failIncrementalSync();
            resyncTable(String.format("IBF for table [%s] is missing from storage so we need to resync it", tableInfo.getTableRef()));
        }
    }
