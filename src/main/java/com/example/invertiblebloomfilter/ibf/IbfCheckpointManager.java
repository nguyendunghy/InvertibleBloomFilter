...
public class IbfCheckpointManager<Adapter extends IbfTableEncoder> {
    ...
    @VisibleForTesting
    public boolean disableMinimumSmallCellCount = false;
    ...
    @VisibleForTesting
    ResizableInvertibleBloomFilter.Sizes currentSize;
    @VisibleForTesting
...
//                        adapter.tableRef().toString());
                break;
            } else if (currentSize != startingSize) {
    ...
    */
    ...
    class DiffManager {
        ...
        void resizePreviousIBF(ResizableInvertibleBloomFilter.Sizes size) {
            if (size == ResizableInvertibleBloomFilter.XLARGE) {
                return;
            }
