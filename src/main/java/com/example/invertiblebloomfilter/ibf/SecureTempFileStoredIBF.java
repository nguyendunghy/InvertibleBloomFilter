...
import static com.example.invertiblebloomfilter.ibf.ResizableInvertibleBloomFilter.computeDivisorPartitionSum;
import static com.example.invertiblebloomfilter.ibf.ResizableInvertibleBloomFilter.convertCellIndex;
...
//        }
        int length = diffIBF.cells.length;
        for (int cellIndex = 0; cellIndex < length; cellIndex++) {
            diffIBF.cells[cellIndex] = term1.ibf.getCell(cellIndex);
            diffIBF.cells[cellIndex].subtract(term2.ibf.getCell(cellIndex));
        }
...
//                writer.write(Base64.getEncoder().encodeToString(cellSerializer.encode(ibf.getCell(cellIndex), null)) + "\n");
            }
...
//            ibf.cells = null;
    ...
    //This method reloads data to cells
