...
public enum SqlStatementUtils {
    ...
    public String columnsForOrderBy(Collection<? extends DbColumnInfo<?>> allKeyColumnInfo) {

        return allKeyColumnInfo
                .stream()
                .sorted(Comparator.comparingInt(DbColumnInfo::orderByQueryPosition))
                .map(ci -> quote(ci.columnName))
                .collect(Collectors.joining(", "));
    }

    @SuppressWarnings({"java:S2583", "java:S4274"})
    public String whereGreaterThan(List<? extends DbColumnInfo<?>> allKeyColumnInfo, List<String> lastEvaluatedKey) {
