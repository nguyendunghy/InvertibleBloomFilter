...
public class OracleIBFQueryBuilder {
    ...
    public OracleIBFQueryBuilder setIbfSizes(ResizingSize ibfSizes) {
    ...
    public String buildPklessQuery() {
        return doBuildPklessQuery(templateParameters());
    }

    private String doBuildPklessQuery(Map<String, Object> templateParameters) {
        return fixedSize
                ? IbfDbUtils.generateInvertibleBloomFilterQuery(IMPORT_TEMPLATE_FILENAME, cellCount, templateParameters)
                : IbfDbUtils.generateResizableInvertibleBloomFilterQuery(
